  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} title="Alterar status">
      ...
      <p className="mt-4 text-xs text-neutral-400">
        {CONTRATO_STATUS_LABEL[status]} só pode avançar para os status acima — transições seguem a state machine de{' '}
        <code className="rounded bg-neutral-100 px-1 dark:bg-white/10">CORE_CONCEPTS.md</code>, validada também no banco
        (nenhuma transição fora dessa lista é aceita mesmo fora desta tela).
      </p>
    </Dialog>
  );
}
