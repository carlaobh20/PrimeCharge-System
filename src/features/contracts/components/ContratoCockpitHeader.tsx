...
import { Car, Check, Copy, FileSignature, MoreHorizontal, Pencil, Plus, User } from 'lucide-react';
...
export function ContratoCockpitHeader({
  contrato,
  usuarioId,
  empresaId,
  onAction,
  ...
  const { data: tags } = useTags('contrato', contrato.id);
    ...
    <header className="animate-cockpit-fade-in overflow-hidden rounded-3xl border border-neutral-200 bg-gradient-to-br from-white to-neutral-50 dark:border-white/10 dark:from-neutral-900 dark:to-neutral-950">
      <div className="flex flex-col gap-6 p-6 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-1 items-start gap-4">
          <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-neutral-200 bg-neutral-100 text-neutral-400 dark:border-white/10 dark:bg-white/5 dark:text-neutral-600">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-2xl font-semibold tracking-tight text-neutral-900 dark:text-neutral-100">
                Contrato {contrato.veiculo?.placa ?? '—'}
              </h1>
              <StatusBadge status={contrato.status} />
            </div>
            <p className="mt-1 flex flex-wrap items-center gap-x-1.5 text-sm text-neutral-500">
              <Link to={`/motoristas/${contrato.motorista_id}`} className="inline-flex items-center gap-1 hover:underline">
                <User className="h-3.5 w-3.5" />
                {contrato.motorista?.nome_completo ?? '—'}
              </Link>
              <span>·</span>
              <Link to={`/veiculos/${contrato.veiculo_id}`} className="inline-flex items-center gap-1 hover:underline">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <FavoritoButton entidadeTipo="contrato" entidadeId={contrato.id} usuarioId={usuarioId} empresaId={empresaId} />
