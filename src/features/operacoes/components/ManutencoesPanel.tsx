...
import { useDeleteManutencao, useManutencoesPorVeiculo } from '../hooks/useManutencoes';
import { NovaManutencaoDialog } from './NovaManutencaoDialog';
import { MANUTENCAO_TIPO_LABEL, type Manutencao, type ManutencaoTipo } from '../types';
...
function ManutencaoRow({ manutencao, onExcluir }: { manutencao: Manutencao; onExcluir: () => void }) {
  return (
    ...
    <div className="space-y-4">
      ...
      {!isLoading && (manutencoes?.length ?? 0) > 0 && (
        <div className="space-y-2">
          {manutencoes!.map((m) => (
            <ManutencaoRow key={m.id} manutencao={m} onExcluir={() => setExcluirId(m.id)} />
          ))}
        </div>
      )}
