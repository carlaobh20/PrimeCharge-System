...
// coletarInteligenciaDaFrota: uma rodada de 4 consultas em lote pra todos os motoristas, não
...
export async function coletarInteligenciaDosMotoristas(motoristas: Motorista[]): Promise<EntityIntelligenceSnapshot[]> {
  ...
  return motoristas.map((motorista) => {
    ...
    const healthScore = calcularHealthScore({ motorista, totalDocumentos, diasAteVencimentoCnh, diasDesdeUltimoEvento });
