        ...
        <ul className="mt-3 space-y-1.5">
          {acoes.map((acao) => (
            <li key={acao.id}>
              <NextActionCard
                acao={acao}
                href={acao.actionKey === 'editar-valores' ? `/veiculos/${veiculoId}/editar` : undefined}
                onClick={acao.actionKey === 'editar-valores' ? undefined : () => onAction(acao.actionKey as ActionKey)}
              />
            </li>
          ))}
        </ul>
