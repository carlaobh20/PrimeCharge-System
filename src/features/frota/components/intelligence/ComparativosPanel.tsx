...
function formatValor(valor: number, unidade: string) {
  if (unidade === 'km') return formatKm(valor);
  ...
  if (resultado.itens.length === 0) {
    return (
      <EmptyState
        ...
        description="Precisa de pelo menos mais um veículo na frota com o mesmo dado preenchido para calcular uma média."
      />
    );
  }
    ...
    <div className="rounded-2xl border border-neutral-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.03]">
      <div className="flex items-baseline justify-between">
        <h3 className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-neutral-500">
          ...
          Comparativo com a frota
        </h3>
        <span className="text-[11px] text-neutral-400">
          {resultado.amostraGrupo} {resultado.amostraGrupo === 1 ? 'outro veículo' : 'outros veículos'}
        </span>
      </div>
